...
alloc_proc(void) {
	 ...
	 */
     ...
     proc->wait_state = 0; //新增
	 proc->cptr = proc->optr = proc->yptr = NULL; //新增
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    local_intr_save(intr_flag);
    {
        proc->pid = get_pid();
        hash_proc(proc);
        //使用set links cptr自己 optr的yptr为自己 yptr为空 并将父进程的cptr调整为自己 pro_num++
        set_links(proc);
    }
    local_intr_restore(intr_flag);
...
load_icode(unsigned char *binary, size_t size) {
    ...
    if (setup_pgdir(mm) != 0) { //将内核的PDT copy过来
        goto bad_pgdir_cleanup_mm;
    }
    ...
    struct proghdr *ph = (struct proghdr *)(binary + elf->e_phoff); //指向段表 ？
    ...
    struct proghdr *ph_end = ph + elf->e_phnum; //最后一个段+1
    for (; ph < ph_end; ph ++) {
    //(3.4) find every program section headers
        if (ph->p_type != ELF_PT_LOAD) { //不加载 跳过
            continue ;
        }
        if (ph->p_filesz > ph->p_memsz) { //段大小大于分配的内存大小
            ret = -E_INVAL_ELF;
            goto bad_cleanup_mmap; //回收
        }
        if (ph->p_filesz == 0) { //段大小为0
            continue ;
        }
    //(3.5) call mm_map fun to setup the new vma ( ph->p_va, ph->p_memsz)
        ...
        if (vm_flags & VM_WRITE) perm |= PTE_W; //页可写
        if ((ret = mm_map(mm, ph->p_va, ph->p_memsz, vm_flags, NULL)) != 0) {// 为该程序段 创建并分配vma
     ...
     //(3.6.1) copy TEXT/DATA section of bianry program 拷贝程序段到内存
        while (start < end) {
    ...
    tf->tf_eflags = 0x00000002 | FL_IF; //默认开中断
    ret = 0;
out:
    return ret;
bad_cleanup_mmap:
...
//在fork之后，mm是父进程的，将mm清空并调用load icode重新填充mm
...
init_main(void *arg) {
    ...
    int pid = kernel_thread(user_main, NULL, 0); //usermain 创建
    if (pid <= 0) {
        panic("create user_main failed.\n");
    }

    while (do_wait(0, NULL) == 0) { //回收僵尸并调度
        schedule();
    }
//执行到这里就不存在用户进程了
